...
def comporIntroducao(collectionCentroPorAno: Collection,collectionCursosPorCentro: Collection, arquivo_intro: str, ano: int, centro_de_ensino: str) -> None:
    """
    ...
    """
    with open(f'./relatorio-components/{arquivo_intro}','r',encoding='utf-8') as f:
        template = f.read()
...
def comporConclusao(collectionNameCursosPorCentro: Collection, arquivo_conclusao: str, ano: int) -> None:
    """
    ...
    """
    with open(f'./relatorio-components/{arquivo_conclusao}', 'r', encoding='utf-8') as f:
        template = f.read()
    renderer = pystache.Renderer()
...
def substituirIdentificadores(filename):
    # Lê o conteúdo do arquivo
    directory = Path('relatorio')
    file = f'{directory}/{filename}'

    with open(f'./relatorio/{filename}', 'r', encoding='utf-8') as file:
        content = file.read()
    ...
    new_content = re.sub(r'Tabela index_', replace_index, content)

    # Salva o novo conteúdo no mesmo arquivo ou em um novo arquivo
    with open(f'./relatorio/{filename}', 'w', encoding='utf-8') as file:
